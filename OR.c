...
void or_reg_assm(void) {
	...
	*/
	...
	// Generally the first parameter should always be a register rd
	if (PARAM1.type != REGISTER) {
		state = MISSING_REG;
		return;
	}

	// This is AND register, so param 2 needs to be a register rs
	if (PARAM2.type != REGISTER) {
		state = MISSING_REG;
		return;
	}

	// This is AND register, so param 3 needs to be a register rt
